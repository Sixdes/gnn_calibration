...
def load_data(name: str, split_type: str, split: int, fold: int) -> Dataset:
    ...
    if name in ['Cora','Citeseer', 'Pubmed']:
        dataset = Planetoid(root=data_pth, name=name, transform=transform)
        load_split_from_numpy_files(dataset, name, split_type, split, fold)
    elif name in ['Computers', 'Photo']:
        dataset = Amazon(root=data_pth, name=name, transform=transform)
        load_split_from_numpy_files(dataset, name, split_type, split, fold)
    elif name in ['CS', 'Physics']:
        dataset = Coauthor(root=data_pth, name=name, transform=transform)
        load_split_from_numpy_files(dataset, name, split_type, split, fold)
    elif name == 'CoraFull':
        dataset = CoraFull(root=data_pth, transform=transform)
        load_split_from_numpy_files(dataset, name, split_type, split, fold)
    return dataset
